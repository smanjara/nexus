...
class restraint_html():
    """Provides a function to copy index.html from restraint created directory to
    workspace.
    """

    def copy_index_html(self):
        """This function copies index.html generated by restraint within the test directory to
        jenkins workspace so that email-ext plugin can pick it up and send it as part of email
        reports.
        """
        idm_config = ConfigParser.SafeConfigParser()
        idm_config.read("etc/global.conf")
        ...
        src = index_html[0]
        dst = os.path.join(workspace_loc, "restraint_results.html")
